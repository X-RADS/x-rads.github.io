...
const strings = {
  zh: {
    skipLink: "跳至主要内容", wordmarkLabel: "X-RADS 首页", primaryNavigationLabel: "主要导航", heroNoteLabel: "参考说明", resultsRegionLabel: "目录结果", "nav.catalog": "查阅", "nav.detail": "详情", heroTitle: "快速找到需要的报告与数据系统", heroLede: "按简称、器官、疾病、模态或分级查阅 RADS。", heroNoteTitle: "专业查阅", heroNote: "先按部位定位，再用筛选缩小范围；详情保留原始英文术语和官方来源。", searchLabel: "搜索条目", searchPlaceholder: "搜索简称、器官、疾病、模态或分级", catalogTitle: "RADS 临床查阅", teamTitle: "团队成员", teamMemberRole: "放射科医师", teamMemberBio: "钟敬宇博士现为上海交通大学医学院附属同仁医院放射科医师。研究方向包括肌肉骨骼和腹部疾病的影像诊断，以及人工智能在医学影像中的应用。", anatomyLabel: "人体部位导航", allAnatomy: "全部", "anatomy.head-neck": "头颈部", "anatomy.chest": "胸部", "anatomy.abdomen-pelvis": "腹部与盆腔", "anatomy.musculoskeletal-whole-body": "骨肌与全身", modalityLabel: "影像模态", statusLabel: "条目状态", frequentLabel: "常用 RADS", allModalities: "全部模态", allStatuses: "全部状态", released: "正式发布", "works-in-progress": "进行中", "in-development": "开发中", "non-acr": "非 ACR", loading: "正在载入参考目录…", loadError: "目录暂时无法载入，请稍后重试。", resultCount: (n) => `共 ${n} 条`, noResults: "没有找到匹配条目。", viewDetail: "查看临床速查", back: "← 返回筛选结果", notFound: "未找到该条目。", organization: "机构", version: "当前版本", releaseDate: "发布日期：", domain: "适用领域", modalities: "影像模态", categoryRange: "分级范围", overview: "临床概览", categories: "评分分级", code: "分级", originalTerm: "原始英文术语", meaning: "临床含义", keyTerms: "关键术语与检查要求", officialSource: "官方来源", openSource: "查看官方原文", noSource: "尚未核对", sourceOnly: "分类定义请查阅原始发表文献。", lastVerified: "上次核对", related: "相关 RADS", professionalNotice: "本站内容仅供专业查阅和教学参考；临床使用应以当前官方版本和所在机构规范为准。", pageTitle: "X-RADS 临床影像参考",
  },
  en: {
    skipLink: "Skip to main content", wordmarkLabel: "X-RADS home", primaryNavigationLabel: "Primary navigation", heroNoteLabel: "Reference note", resultsRegionLabel: "Catalog results", "nav.catalog": "Browse", "nav.detail": "Detail", heroTitle: "Find the reporting and data system you need", heroLede: "Browse RADS by acronym, organ, disease, modality, or category.", heroNoteTitle: "Professional reference", heroNote: "Start by anatomy, then narrow results with filters. Detail pages retain original English terms and official sources.", searchLabel: "Search entries", searchPlaceholder: "Search acronym, organ, disease, modality, or category", catalogTitle: "RADS clinical reference", teamTitle: "Team", teamMemberRole: "Radiologist", teamMemberBio: "Dr. Jingyu Zhong works as a radiologist at Tongren Hospital, Shanghai Jiao Tong University School of Medicine. His research interests include the diagnosis of musculoskeletal and abdominal diseases, as well as the application of artificial intelligence in medical imaging.", anatomyLabel: "Anatomy navigator", allAnatomy: "All", "anatomy.head-neck": "Head & neck", "anatomy.chest": "Chest", "anatomy.abdomen-pelvis": "Abdomen & pelvis", "anatomy.musculoskeletal-whole-body": "Musculoskeletal & whole body", modalityLabel: "Modality", statusLabel: "Entry status", frequentLabel: "Frequently used", allModalities: "All modalities", allStatuses: "All statuses", released: "Released", "works-in-progress": "Works in progress", "in-development": "In development", "non-acr": "Non-ACR", loading: "Loading reference catalog…", loadError: "The catalog could not be loaded. Please try again later.", resultCount: (n) => `${n} entries`, noResults: "No matching entries.", viewDetail: "Open clinical reference", back: "← Back to filtered results", notFound: "Entry not found.", organization: "Organization", version: "Current version", releaseDate: "Release date:", domain: "Application domain", modalities: "Modalities", categoryRange: "Category range", overview: "Clinical overview", categories: "Categories", code: "Code", originalTerm: "Original English term", meaning: "Clinical meaning", keyTerms: "Key terms and examination conditions", officialSource: "Official source", openSource: "Open official source", noSource: "Not verified", sourceOnly: "See the original publication for category definitions.", lastVerified: "Last verified", related: "Related RADS", professionalNotice: "This site is for professional reference and teaching only. Current official guidance and institutional policy take precedence in clinical use.", pageTitle: "X-RADS Clinical Reference",
  },
};
...
const teamCopy = {
  zh: {
    ...
    teamMemberAffiliation: "上海交通大学医学院附属同仁医院",
    teamMemberRole: "放射科医师",
    teamMemberBio: "钟京渝博士现为上海交通大学医学院附属同仁医院放射科医师。研究方向包括肌肉骨骼和腹部疾病的影像诊断，以及人工智能在医学影像中的应用。",
    teamEmailLabel: "发送邮件联系钟京渝",
  },
  en: {
    teamMemberName: "Jingyu Zhong, MD",
    teamMemberAffiliation: "Tongren Hospital, Shanghai Jiao Tong University School of Medicine",
    teamMemberRole: "Radiologist",
    teamMemberBio: "Dr. Jingyu Zhong works as a radiologist at Tongren Hospital, Shanghai Jiao Tong University School of Medicine. His research interests include the diagnosis of musculoskeletal and abdominal diseases, as well as the application of artificial intelligence in medical imaging.",
    teamEmailLabel: "Email Jingyu Zhong",
  },
};

const anatomyGroups = {
  "head-neck": ["head-neck", "thyroid", "neck-node", "carotid"],
  chest: ["breast", "lung", "heart"],
  "abdomen-pelvis": ["colon", "liver", "ovary", "prostate", "acute-abdomen", "appendix", "gallbladder", "stomach"],
...
export function renderDetail(records, url) {
  ...
  const publicationDetailNote = source ? text(state.lang, "sourceOnly") : text(state.lang, "noSource");
  const rows = item.categories.length ? item.categories.map((category) => `<tr><td>${escapeHtml(withFallback(state.lang, category.code))}</td><td>${escapeHtml(withFallback(state.lang, category.original))}</td><td>${escapeHtml(withFallback(state.lang, category.displayMeaning))}</td></tr>`).join("") : `<tr><td colspan="3">${escapeHtml(publicationDetailNote)}</td></tr>`;
  const terms = item.originalTerms.length ? item.originalTerms.map((term) => `<li><strong>${escapeHtml(withFallback(state.lang, term.term))}</strong> — ${escapeHtml(withFallback(state.lang, term.displayExplanation))}</li>`).join("") : `<li>${escapeHtml(publicationDetailNote)}</li>`;
